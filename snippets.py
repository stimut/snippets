...
jinja2.default_config["filters"] = {
    "readable_date": (lambda value: value.strftime("%B %d, %Y").replace(" 0", " ")),
    "iso_date": (lambda value: value.strftime("%m-%d-%Y")),
}
...
class UserPage(BaseHandler):
    ...
    def get(self):
        ...
        snippets = snippet.fill_in_missing_snippets(snippets, user, user_email, _TODAY_FN())
        ...
        template_values = {
            ...
            "editable": (
                _logged_in_user_has_permission_for(user_email)
    ...
    def get(self):
        ...
        for snippet in snippets:
            ...
            if snippet.private and not _can_view_private_snippets(
                ...
                continue
            category = email_to_category.get(snippet.email, user.NULL_CATEGORY)
...
class UpdateSnippet(BaseHandler):
    ...
    def post(self):
        ...
        if not users.get_current_user():
            ...
            self.response.out.write('{"status": 403, ' '"message": "not logged in"}')
            return
        ...
        if not _logged_in_user_has_permission_for(email):
            ...
            self.response.set_status(403)
            error = "You do not have permissions to update user snippets for %s" % email
            self.response.out.write('{"status": 403, ' '"message": "%s"}' % error)
            return
    ...
    def get(self):
        ...
        if not _logged_in_user_has_permission_for(email):
            ...
            raise RuntimeError("You do not have permissions to update user snippets for %s" % email)
...
class Settings(BaseHandler):
    ...
    def get(self):
        ...
        if not _logged_in_user_has_permission_for(user_email):
            # TODO(csilvers): return a 403 here instead.
            raise RuntimeError(
                "You do not have permissions to view user settings for %s" % user_email
            )
...
class UpdateSettings(BaseHandler):
    ...
    def get(self):
        ...
        if not _logged_in_user_has_permission_for(user_email):
            # TODO(csilvers): return a 403 here instead.
            raise RuntimeError(
                "You do not have permissions to modify user settings for %s" % user_email
            )
        ...
        if self.request.get("hide"):
            ...
            db.delete(user)
            self.redirect(
                "/weekly?msg=Your+account+has+been+deleted.+"
        ...
        if redirect_to == "snippet_entry":  # true for new_user.html
            ...
            self.redirect("/settings?msg=Changes+saved&u=%s" % urllib.quote(user_email))
...
class AppSettings(BaseHandler):
    ...
    def get(self):
        ...
        app_settings = AppSettings.get(create_if_missing=True, domains=[my_domain])
...
class UpdateAppSettings(BaseHandler):
    ...
    def get(self):
        ...
        @db.transactional
        def update_settings():
            app_settings = AppSettings.get(create_if_missing=True, domains=domains)
        ...
        if redirect_to == "user_setting":  # true for new_user.html
            self.redirect(
                "/settings?redirect_to=snippet_entry&msg=Now+enter+your+personal+user+settings."
            )
        else:
    ...
    def get(self):
        ...
        for (name, value) in self.request.params.iteritems():
            if name.startswith("hide "):
                ...
                self.redirect(
                    "/admin/manage_users?sort_by=%s&msg=%s+hidden"
            ...
            if name.startswith("delete "):
                ...
                time.sleep(0.1)  # encourage eventual consistency
                self.redirect(
                    "/admin/manage_users?sort_by=%s&msg=%s+deleted"
        ...
        for user in results:
            ...
            if last_snippet:
                seconds_since_snippet = (_TODAY_FN().date() - last_snippet.week).total_seconds()
                weeks_since_snippet = int(
                    seconds_since_snippet / datetime.timedelta(days=7).total_seconds()
                )
            else:
                weeks_since_snippet = None
            user_data.append((user.email, user.is_hidden, user.created, weeks_since_snippet))
        ...
        if sort_by == "email":
            ...
            user_data.sort(
                lambda x, y: (
                    -cmp(x[2] or datetime.datetime.min, y[2] or datetime.datetime.min)
                    or cmp(x[0], y[0])
                )
            )
            ...
            user_data.sort(
                lambda x, y: (
                    -cmp(1000 if x[3] is None else x[3], 1000 if y[3] is None else y[3])
                    or cmp(x[0], y[0])
                )
            )
        else:
...
def _maybe_send_snippets_mail(to, subject, template_path, template_values):
    ...
    mail.send_mail(
        sender=app_settings.email_from,
        to=to,
        subject=subject,
        body=jinja2_instance.render_template(template_path, **template_values),
    )
...
class SendReminderEmail(BaseHandler):
    ...
    def _send_mail(self, email):
        template_values = {}
        _maybe_send_snippets_mail(
            email, "Weekly snippets due today at 5pm", "reminder_email.txt", template_values
        )
    ...
    def get(self):
        ...
        for (user_email, has_snippet) in email_to_has_snippet.iteritems():
            if not has_snippet:
                ...
                logging.debug(
                    "did not send reminder email to %s: has a snippet already" % user_email
                )
...
class SendViewEmail(BaseHandler):
    ...
    def _send_mail(self, email, has_snippets):
        ...
        _maybe_send_snippets_mail(
            email, "Weekly snippets are ready!", "view_email.txt", template_values
        )
