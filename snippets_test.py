...
class SnippetsTestBase(unittest.TestCase):
    def setUp(self):
        ...
        self.slack_sends = []
        slacklib.send_to_slack_channel = lambda *args: self.slack_sends.append(args)
    ...
    def login(self, email):
        ...
        settings = AppSettings.get(
            create_if_missing=True, domains=["example.com", "some_other_domain.com"],
        )
    ...
    def assertNumSnippets(self, body, expected_count):
        ...
        self.assertEqual(expected_count, body.count("unique-snippet"), body)
    ...
    def _ith_snippet(self, body, snippet_number):
        ...
        try:
            return body.split("unique-snippet", snippet_number + 2)[snippet_number + 1]
        except IndexError:
            raise IndexError("Has fewer than %d snippets:\n%s" % ((snippet_number + 1), body))
...
class PostTestCase(SnippetsTestBase):
    ...
    def testPostSnippet(self):
        self.login("user@example.com")
        url = "/update_snippet"
        params = {"week": "02-20-2012", "snippet": "my inspired snippet"}
    ...
    def testPostSnippetNotLoggedIn(self):
        url = "/update_snippet"
        params = {"week": "02-20-2012", "snippet": "my fallacious snippet", "u": "user@example.com"}
        response = self.request_fetcher.post(url, params, status=403)
        self.assertIn('"status": 403', response)
    ...
    def testPostSnippetIsolation(self):
        ...
        url = "/update_snippet"
        params = {"week": "02-27-2012", "snippet": "updated second snippet"}
...
class AccessTestCase(UserTestBase):
    ...
    def testCannotEditOtherSnippets(self):
        url = "/update_snippet?week=02-20-2012&snippet=my+snippet&u=notuser@example.com"
    ...
    def testCanEditOwnSnippets(self):
        url = "/update_snippet?week=02-20-2012&snippet=my+snippet&u=user@example.com"
        self.request_fetcher.get(url)
...
class NewUserTestCase(UserTestBase):
    ...
    def testNewUserContinueUrl(self):
        ...
        self.assertIn('name="redirect_to" value="snippet_entry"', settings_response.body)
    ...
    def testNewAdminContinueUrls(self):
        ...
        # Now kinda-simulate clicking on the submit button.
        done_response = self.request_fetcher.get(
            "/admin/update_settings?domains=example.com&redirect_to=%s" % continue_url
        )
        if done_response.status_int in (301, 302, 303, 304):
            done_response = done_response.follow()
        self.assertIn("<title>User settings", done_response.body)
        self.assertIn('name="redirect_to" value="snippet_entry"', done_response.body)
    ...
    def testNewUserInheritsAppDefaults(self):
        ...
        self.assertRegexpMatches(settings_response.body, r'name="markdown"\s+value="yes"\s+checked')
        self.assertRegexpMatches(settings_response.body, r'name="private"\s+value="yes"\s+checked')
        ...
        self.assertRegexpMatches(settings_response.body, r'name="markdown"\s+value="no"\s+checked')
        self.assertRegexpMatches(settings_response.body, r'name="private"\s+value="no"\s+checked')
...
class AppSettingsTestCase(UserTestBase):
    ...
    def testDomainsParsing(self):
        self.request_fetcher.get("/admin/update_settings?domains=a.com,b.com++c.com%0Bd.com,%0B")
        app_settings = AppSettings.get()
        self.assertEqual(["a.com", "b.com", "c.com", "d.com"], app_settings.domains)
...
class UserSettingsTestCase(UserTestBase):
    ...
    def assertInputIsChecked(self, name, body, snippet_number):
        self.assertRegexpMatches(
            body, r'name="%s" value="True"\s+checked\s*>' % name, snippet_number
        )

    def assertInputIsNotChecked(self, name, body, snippet_number):
        self.assertRegexpMatches(body, r'name="%s" value="True"\s*>' % name, snippet_number)
    ...
    def testPrivateUser(self):
        self.request_fetcher.get("/update_settings?u=user@example.com&private=yes")
        response = self.request_fetcher.get("/")
        self.assertInputIsChecked("private", response.body, 0)
        self.assertInputIsNotChecked("is_markdown", response.body, 0)

    def testMarkdownUser(self):
        self.request_fetcher.get("/update_settings?u=user@example.com&markdown=yes")
        response = self.request_fetcher.get("/")
        self.assertInputIsNotChecked("private", response.body, 0)
    ...
    def testCategoryUnset(self):
        ...
        self.assertInSnippet(
            '<strong>WARNING:</strong> Snippet will go in the "(unknown)"', response.body, 0
        )

    def testCategorySet(self):
        self.request_fetcher.get("/update_settings?u=user@example.com")
        self.request_fetcher.get("/update_settings?u=user@example.com&category=Dev")
        response = self.request_fetcher.get("/")
    ...
    def testCategoryCheckForFilledInSnippets(self):
        ...
        self.assertNumSnippets(response.body, 53)
        self.assertInSnippet(
            '<strong>WARNING:</strong> Snippet will go in the "(unknown)"', response.body, 9
        )
        self.assertInSnippet("old snippet", response.body, 52)
        self.assertInSnippet(
            '<strong>WARNING:</strong> Snippet will go in the "(unknown)"', response.body, 52
        )
...
class SetAndViewSnippetsTestCase(UserTestBase):
    ...
    def testUrlize(self):
        ...
        self.assertNumSnippets(response.body, 1)
        self.assertInSnippet(
            '>visit <a href="http://foo.com">http://foo.com</a><', response.body, 0
        )
...
class MarkdownSnippetTestCase(UserTestBase):
    ...
    def setUp(self):
        ...
        url = "/update_snippet?week=02-13-2012&snippet=*+item+1%0A*+item+2&is_markdown=True"
        self.request_fetcher.get(url)
...
class ManageUsersTestCase(UserTestBase):
    ...
    def testSortByEmail(self):
        response = self.request_fetcher.get("/admin/manage_users?sort_by=email")
        expected = ["has_many_snippets", "has_no_snippets", "has_old_snippet", "has_one_snippet"]
    ...
    def testSortByCreation(self):
        response = self.request_fetcher.get("/admin/manage_users?sort_by=creation_time")
        # Reverse order from when we created them above.
        expected = ["has_no_snippets", "has_old_snippet", "has_many_snippets", "has_one_snippet"]
        self.assertEqual(expected, self.get_user_list(response.body))
        self.assertNotIn("@example.com deleted", response.body)  # we didn't

    def testSortByLastSnippet(self):
        response = self.request_fetcher.get("/admin/manage_users?sort_by=last_snippet_time")
    ...
    def testBadSortBy(self):
        ...
        self.request_fetcher.get("/admin/manage_users?sort_by=unknown", status=500)
    ...
    def testPreserveSortBy(self):
        response = self.request_fetcher.get(
            "/admin/manage_users?hide+has_old_snippet@example.com=Hide&sort_by=last_snippet_time"
        )
        if response.status_int in (301, 302, 303, 304):
            response = response.follow()
    ...
    def testUnhide(self):
        self.request_fetcher.get("/admin/manage_users?hide+has_old_snippet@example.com=Hide")
        response = self.request_fetcher.get(
    ...
    def testInvalidButton(self):
        self.request_fetcher.get("/admin/manage_users?delete+has_old_snippet=Delete", status=500)
        self.request_fetcher.get("/admin/manage_users?hide+has_old_snippet=Hide", status=500)
        self.request_fetcher.get("/admin/manage_users?unhide+has_old_snippet=Unhide", status=500)
...
class SendingEmailTestCase(UserTestBase):
    ...
    def testDefaultEmailFrom(self):
        app_settings = AppSettings.get()
        self.assertEqual("Snippet Server <user+snippets@example.com>", app_settings.email_from)
    ...
    def testSendReminderEmail(self):
        ...
        self.assertEmailContains("has_no_snippets@example.com", "https://example.com")
    ...
    def testSendViewEmail(self):
        ...
        self.assertEmailDoesNotContain("has_snippet@example.com", "not too late")
        self.assertEmailDoesNotContain("has_many_snippets@example.com", "not too late")
        self.assertEmailContains("does_not_have_snippet@example.com", "not too late")
        self.assertEmailContains("has_no_snippets@example.com", "not too late")
        self.assertEmailContains("does_not_have_snippet@example.com", "https://example.com")
    ...
    def testEmailQuotas(self):
        ...
        def count_calls_per_minute(sleep_seconds):
            ...
            self.max_calls_per_minute = max(self.max_calls_per_minute, self.calls_this_minute)
        ...
        users = [User(email="snippets%d@example.com" % i) for i in xrange(500)]
        db.put(users)

        self.request_fetcher.get("/admin/send_view_email")
        # https://developers.google.com/appengine/docs/quotas#Mail
        self.assertTrue(
            self.max_calls_per_minute <= 32, "%d <= %d" % (self.max_calls_per_minute, 32)
        )
        # Make sure we're not too slow either: say 1/2.5 seconds on average.
        self.assertTrue(
            self.total_sleep_seconds <= len(users) * 2.5,
            "%d <= %d" % (self.total_sleep_seconds, len(users) * 2.5),
        )
...
class TitleCaseTestCase(unittest.TestCase):
    def testSimple(self):
        self.assertEqual("A Word to the Wise", snippets._title_case("a word to the wise"))
...
class DisplayNameTestCase(UserTestBase):
    ...
    def testUserHasDisplayName(self):
        self.request_fetcher.get("/update_settings?u=user@example.com&display_name=test+name")
        url = "/update_snippet?week=02-20-2012&snippet=my+snippet"
        self.request_fetcher.get(url)
        response = self.request_fetcher.get("/weekly?week=02-20-2012")
        self.assertNumSnippets(response.body, 1)
        self.assertInSnippet("<h3>test name (user@example.com):</h3>", response.body, 0)
    ...
    def testUserChangesDisplayName(self):
        ...
        self.request_fetcher.get("/update_settings?u=user@example.com&display_name=fancy+name")
        response = self.request_fetcher.get("/weekly?week=02-20-2012")
        self.assertNumSnippets(response.body, 1)
        self.assertInSnippet("<h3>fancy name (user@example.com):</h3>", response.body, 0)

    def testSnippetHasDisplayName(self):
        self.request_fetcher.get("/update_settings?u=user@example.com&display_name=test+name")
        url = "/update_snippet?week=02-20-2012&snippet=my+snippet"
        self.request_fetcher.get(url)
        self.request_fetcher.get("/update_settings?u=user@example.com&display_name=")
        response = self.request_fetcher.get("/weekly?week=02-20-2012")
        self.assertNumSnippets(response.body, 1)
        self.assertInSnippet("<h3>test name (user@example.com):</h3>", response.body, 0)
